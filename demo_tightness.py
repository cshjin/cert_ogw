...
for n in tqdm(np.arange(0, 51, 5), desc="budget"):
    for i in tqdm(range(100), desc="round", leave=False):
        A_ = random_perturb(A, n=n, seed=i)
        C_ = dist(A_)
        # C_ = C_ / C_.max()
        gwtil_lb_fval = gwtil_lb(C_, C)
        gwtil_ub_fval = gwtil_ub(C_, C)
...
plt.plot(xs, lb_means, color='#e41a1c', label=r"$\widetilde{GW}_{lb}$")
plt.xticks(range(1, len(xs) + 1), xs)
plt.xlabel(r"$\delta_g$")
plt.margins(0, 0)
plt.legend()
plt.tight_layout()
plt.savefig("tmp_BZR.png")
# plt.show()
