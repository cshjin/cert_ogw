...
from fgw.optim import NonConvergenceError
from fgw.ot_distances import (Approx_Fused_Gromov_Wasserstein_distance,
    ...
    def compute_all_distance(self, X, Y, matrix=None):
        ...
        if matrix is not None:
            ...
            if np.all(X == Y):
                ...
                # REVIEW: for some reason it prevents calculating D in one line
                _ = D + D.T
                __ = _ - H
                D = __
                # D=D+D.T-H
...
class Graph_FGW_SVC_Classifier(GenericSVCClassifier):
    ...
    def __init__(self,
        ...
        self.gw = Fused_Gromov_Wasserstein_distance(
            alpha=alpha, method=method, features_metric=features_metric, amijo=amijo)
    ...
    def set_params(self, **parameters):
        ...
        gw2 = Fused_Gromov_Wasserstein_distance(
            alpha=self.alpha,
            method=self.method,
            features_metric=self.features_metric,
            amijo=self.amijo)
        if self.gw.get_tuning_params() != gw2.get_tuning_params(
        ):  # if not same tuning param recreate a new FGW object because the similarity measure changes
            self.gw = Fused_Gromov_Wasserstein_distance(
                alpha=self.alpha,
                method=self.method,
                features_metric=self.features_metric,
...
class Graph_Approx_FGW_SVC_Classifier(GenericSVCClassifier):
    """ Graph_Approx_FGW_SVC_Classifier is a generic class that inherit from GenericSVCClassifier.
    It uses the FGW approximation in different domain as similarity measure

    Approx. Attributes:
    ----------
    fenchel : bool
              If True, use fenchel dual of the approx. distance
    domain : string
             "NE", use exact fused Gromov Wasserstein distance, optimzing over domian N \\cap E
             "P", use approx. fused Gromov Wasserstein distance, optimzing over domian N \\cap E \\cap O
             "O", use approx. fused Gromov Wasserstein distance, optimzing over domian O
             "OE", use approx. fused Gromov Wasserstein distance, optimzing over domian O \\cap E
    path : string
           If method=='linear_approxmation', load pre-computed linear approximation mapping location.
    ...
    def __init__(
        ...
        if domain in ["NE"]:
            self.gw = Fused_Gromov_Wasserstein_distance(
                alpha=alpha, method=method, features_metric=features_metric, amijo=amijo)
        elif domain in ['OE', 'OES', 'OER', 'entropic']:
            # REVIEW: How do we calculate the Approx. FGW with commute time (i.e. without linear approximation?)
            self.gw = Approx_Fused_Gromov_Wasserstein_distance(
                alpha=alpha, method=method, features_metric=features_metric, amijo=amijo, domain=domain)
        else:
        ...
        for x in self._fit_X:
            if x.C is None or x.name_struct_dist != self.method:
                if self.verbose:
                    ...
                    print('******************************************************')
                _ = x.distance_matrix(method=self.method, force_recompute=True)
        super(Graph_Approx_FGW_SVC_Classifier, self).fit(X, y, matrix)
    ...
    def set_params(self, **parameters):
        ...
        self.set_one_param(parameters, "amijo")
        self.set_one_param(parameters, "wl")
        self.set_one_param(parameters, "domain")
        self.svc = SVC(C=self.C, kernel="precomputed", verbose=self.verbose, max_iter=10000000)
        ...
        if self.domain in ["NE"]:
            gw2 = Fused_Gromov_Wasserstein_distance(
                alpha=self.alpha,
                method=self.method,
                features_metric=self.features_metric,
                amijo=self.amijo)
        elif self.domain in ['OE', 'OES', 'OER', 'entropic']:
            gw2 = Approx_Fused_Gromov_Wasserstein_distance(
                alpha=self.alpha,
                method=self.method,
                features_metric=self.features_metric,
                amijo=self.amijo,
    ...
    def get_distances_params(self):
        ...
        dall.update(self.gw.get_tuning_params())
        dall.update({'wl': self.wl})
        dall.update({'domain': self.domain})
        return dall
