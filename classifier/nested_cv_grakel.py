...
import os
import argparse
from fgw.utils import str2bool, setup_logger, unique_repr, split_train_test
from fgw.gk_classifier import GK_classifier
...
def explode_tuned_parameters(tuned_params):
    ...
    combinations = [dict(zip(varNames, prod)) for prod in itertools.product(
        *(tuned_parameters2[varName] for varName in varNames))]
...
def nested_cv(G, y, tuned_parameters, logging, n_iter=10, n_inner=10, verbose=1):
    ...
    for i in range(n_iter):
        k_fold = StratifiedKFold(n_splits=n_inner, shuffle=True, random_state=i)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-r', '--result_directory', nargs='?', default="../results", type=str,
    ...
    parser.add_argument('-C', '--Csvm', nargs='?', default=-1, type=float,
                        help='C parameter in Linear SVM. If not specified cross validated')
    parser.add_argument('-hwl', '--hwl', nargs='?', default=-1, type=int,
                        help='h parameter in WL. If not specified cross validated')
    parser.add_argument('-k', '--gkk', nargs='?', default=-1, type=int,
                        help='k parameter in GK. If not specified cross validated')
    parser.add_argument('-l', '--lambd', nargs='?', default=-1, type=int,
                        help='lambda parameter in RW. If not specified cross validated')
    ...
    if args.model_name == 'wl':
        tuned_parameters = [{'C': Clist, 'kernel_params': [
            [{"name": "weisfeiler_lehman", "n_iter": x}, {"name": "subtree_wl"}] for x in hlist], 'normalize': [True]}]
        nested_cv(G, y, tuned_parameters, logger, n_iter=args.n_outer, n_inner=args.n_inner, verbose=args.verbose)
    if args.model_name == 'sp':
        tuned_parameters = [{'C': Clist, 'kernel_params': [
            [{"name": "shortest_path", "with_labels": args.use_lab}]], 'normalize': [True]}]
        nested_cv(G, y, tuned_parameters, logger, n_iter=args.n_outer, n_inner=args.n_inner, verbose=args.verbose)
    if args.model_name == 'gk':
        S = [{"delta": 0.1, "epsilon": 0.1}, {"delta": 0.05, "epsilon": 0.1},
             {"delta": 0.05, "epsilon": 0.05}, {"delta": 0.1, "epsilon": 0.05}]
        tuned_parameters = [{'C': Clist, 'kernel_params': [[{"name": "graphlet_sampling", "k": kx,
                                                             "sampling": sx}] for kx, sx in itertools.product(gkklist, S)], 'normalize': [True]}]
        nested_cv(G, y, tuned_parameters, logger, n_iter=args.n_outer, n_inner=args.n_inner, verbose=args.verbose)
    if args.model_name == 'rw':
        tuned_parameters = [{'C': Clist, 'kernel_params': [
            [{"name": "random_walk", "with_labels": args.use_lab, "lamda": l}] for l in lambdlist], 'normalize': [True]}]
        nested_cv(G, y, tuned_parameters, logger, n_iter=args.n_outer, n_inner=args.n_inner, verbose=args.verbose)
    if args.model_name == 'hopper':
        tuned_parameters = [{'C': Clist, 'kernel_params': [
            [{"name": "graph_hopper", "kernel_type": 'linear'}]], 'normalize': [True]}]
        nested_cv(G, y, tuned_parameters, logger, n_iter=args.n_outer, n_inner=args.n_inner, verbose=args.verbose)
    if args.model_name == 'propa':
        tmaxlist = [1, 3, 5, 8, 10, 15, 20]
        tuned_parameters = [{'C': Clist, 'kernel_params': [
            [{"name": "propagation", "with_attributes": True, "t_max": x}] for x in tmaxlist], 'normalize': [True]}]
        nested_cv(G, y, tuned_parameters, logger, n_iter=args.n_outer, n_inner=args.n_inner, verbose=args.verbose)
