...
import numpy as np
import ast
from fgw.utils import split_train_test, unique_repr, create_log_dir
from sklearn.metrics import accuracy_score
from fgw.custom_svc import Graph_FGW_SVC_Classifier, Graph_Approx_FGW_SVC_Classifier
import time
import fgw.utils as utils
import argparse
...
def nested_fgw(
    ...
    filtre = set(Graph_Approx_FGW_SVC_Classifier().get_distances_params().keys())
    ...
    for params in all_params_filtered:
        ...
        clf = Graph_Approx_FGW_SVC_Classifier(**params)
        if path is None:
            if verbose > 1:
    ...
    for i in range(n_iter):  # do the nested CV
        ...
        G_train, y_train, idx_train, G_test, y_test, idx_test = split_train_test(
            list(zip(X, list(y))), ratio=0.9, seed=i)
        ...
        for idx_subtrain, idx_valid in k_fold.split(G_train, y_train):
            ...
            for param in allparams:
                ...
                # clf = Graph_Approx_FGW_SVC_Classifier()
...
if __name__ == '__main__':
    ...
    parser.add_argument('-r', '--log_dir', nargs='?', default="../log", type=str,
    ...
    parser.add_argument(
        ...
        nargs='?',
        type=int,
        help='Use the Weisfeler Lehman features if wl>0 ',
        default=0)
    parser.add_argument(
        '-at',
        '--attributes',
        nargs='?',
        help='wether to use vector attributes of the graph',
        type=utils.str2bool,
        default=True)
    parser.add_argument(
        '-fea',
    ...
    parser.add_argument(
        ...
        '--amijo',
        nargs='?',
        help='wether to use amijo linesearch',
        type=utils.str2bool,
        default=True)
    parser.add_argument(
        '-a',
        ...
        nargs='+',
        type=float,
        help='Alphas to cross validate. Ignored if cva is true',
        default=-8000)
    parser.add_argument(
        '-cva',
        '--automatic_cv_alpha',
        nargs='?',
        help='wether to use a predifined CV grid for alpha. 15 alphas are tested',
        type=utils.str2bool,
        default=False)
    parser.add_argument(
        '-dom',
    ...
    name = 'fgw' + '_' + args.dataset_name + '_' + args.feature_metric + '_' + args.structure_metric
    ...
    log_dir = create_log_dir(args, name)
    ...
    tuned_parameters = [{'alpha': alpha_list,
                         ...
                         'method':[args.structure_metric],
                         'wl':[args.wl_feature],
                         'amijo':[args.amijo],
                         'domain':[args.domain]}]

    nested_fgw(X, y, tuned_parameters, args.dataset_name, logger, args.distances_path, n_inner=args.n_inner,
               n_iter=args.n_outer, verbose=args.verbose, domain=args.domain, optionnal=str(args.optionnal_name))
